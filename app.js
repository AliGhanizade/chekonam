...
const state = {manifest:null, catalog:[], datasets:{}, activeCategory:null, activeItems:[], activeCategoryId:null, isIdeaList:false, lastSelections:{}, currentIndex:0, mode:'cards', cardsLocked:false, appearance:{theme:'neon',scheme:'dark',depth:'2d',accent:'#d8f36b',secondary:'#72d8d2',radius:18,colorsTouched:false}, settings:{effects:true,surprise:true}, stats:{visits:0,choices:0,custom:0,byDay:{}}};
let masterGain = null;
...
async function init(){
  ...
  renderCategories(); bindEvents(); applyAppearance(); if(window.matchMedia&&!state.systemMedia){state.systemMedia=window.matchMedia('(prefers-color-scheme:light)');state.systemMedia.addEventListener('change',()=>{if(state.appearance.scheme==='system')applyAppearance();});} updateAdmin();
  track('visit');
}
...
function bindEvents(){
  ...
  $('#resetAppearance').addEventListener('click', () => { state.appearance={theme:'neon',scheme:'dark',depth:'2d',accent:'#d8f36b',secondary:'#72d8d2',radius:18,colorsTouched:false}; saveAppearance(); applyAppearance(); showToast('ظاهر به حالت اولیه برگشت'); });
...
async function openCategory(id){
  ...
  state.activeCategory = category; state.activeItems = state.datasets[id].items || fallbackData.items; state.currentIndex = 0; state.isIdeaList = category.id==='photo' || /ایده|عکاسی/.test(category.title);
...
function renderOptionCards(){
  const hint=state.isIdeaList?'فقط خط بزن · حداکثر ۲۲ مورد': 'برای انتخاب یا خط‌زدن لمس کن'; $('#optionCards').innerHTML = `<div class="list-heading"><span>لیست گزینه‌ها</span><small>${hint}</small></div>${state.activeItems.map((item,index) => `<button class="option-card" data-option-index="${index}" type="button"><span class="option-check">✓</span><span>${item.title}</span></button>`).join('')}`;
  $$('#optionCards .option-card').forEach(card => card.addEventListener('click', () => markListOption(Number(card.dataset.optionIndex),card)));
}
function markListOption(index,card){if(state.mode==='cards'&&state.cardsLocked&&!state.isIdeaList)return showToast('این دور قفل شده؛ برای انتخاب دوباره «کارت‌های تازه» را بزن');if(state.isIdeaList&&!card.classList.contains('done')&&$$('#optionCards .option-card.done').length>=22)return showToast('برای این لیست حداکثر ۲۲ مورد را می‌توانی خط بزنی');card.classList.toggle('done');if(card.classList.contains('done')&&!state.isIdeaList)choose(index);}
...
function setMode(mode){
  ...
  const cards=mode==='cards', auto=mode==='auto', scratch=mode==='scratch', box=mode==='box';
  $('#mysteryStage').hidden=!cards; $('#autoStage').hidden=!auto; $('#scratchStage').hidden=!scratch; $('#boxStage').hidden=!box; $('#optionCards').hidden=false;
  const copy={cards:['سه کارت ناشناس','تعداد کارت‌ها را انتخاب کن و یکی را لمس کن؛ بعد از انتخاب، همان دور قفل می‌شود.','کارت‌های تازه ↻'],auto:['انتخاب خودکار','من از بین همه‌ی گزینه‌ها یک پیشنهاد مناسب برایت پیدا می‌کنم.','پیدا کن ✦'],scratch:['بخت‌آزمایی دستی','خانه‌ها را با انگشت یا ماوس خراش بده تا گزینه‌ها یکی‌یکی نمایان شوند.','شروع صفحه‌ی تازه ⌁'],box:['جعبه‌ی نورانی','چهار ضربه بزن، جعبه باز می‌شود و یک انتخاب از داخلش بیرون می‌آید.','باز کردن جعبه □']}[mode];
  $('#resultKicker').textContent=copy[0]; $('#resultDescription').textContent=copy[1]; $('#spinButton').innerHTML=copy[2];
  if(cards) renderMysteryCards(); if(auto) $('#autoStage strong').textContent='در حال پیدا کردن...'; if(scratch) renderScratchCard(); if(box) resetBox();
}
function runMode(){ if(state.mode==='cards') return renderMysteryCards(); if(state.mode==='auto') return autoPick(); if(state.mode==='scratch') return renderScratchCard(); return openBox(true); }
function autoPick(){if(state.autoTimer||!state.activeItems.length)return;let ticks=0;$('#spinButton').disabled=true;state.autoTimer=setInterval(()=>{const item=state.activeItems[Math.floor(Math.random()*state.activeItems.length)];$('#autoStage strong').textContent=item.title;ticks++;if(ticks>=9){clearInterval(state.autoTimer);state.autoTimer=null;$('#spinButton').disabled=false;choose(state.activeItems.indexOf(item));}},100);}
